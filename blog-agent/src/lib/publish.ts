// In-repo publisher. The static site lives at the ROOT of this repo
// (index.html / styles.css), so posts are written to <root>/blog/<slug>/
// (index.html + hero.jpg), the blog index to <root>/blog/index.html, and
// sitemap.xml at the root. No cross-repo clone/token needed — the GitHub
// Actions workflow commits and pushes, and the host connected to this repo
// auto-deploys.
...
// The site root is the repo root, one level up from blog-agent/ (process.cwd()).
const SITE_DIR = resolve(process.cwd(), "..");
...
/**
 * Rebuilds sitemap.xml from the homepage, the blog index, and every
...
/** Guard: fail early with a clear message if the site root isn't where we expect. */
export function assertSiteDir(): void {
  if (!existsSync(resolve(SITE_DIR, "index.html"))) {
    throw new Error(
      `Expected the site's index.html at ${SITE_DIR} but it doesn't exist. ` +
        `Run the blog agent from the blog-agent/ directory of the site repo.`
    );
  }
}
