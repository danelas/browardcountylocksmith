...
async function main(): Promise<void> {
  ...
  if (DRY_RUN) {
    console.log("[blog] dry-run — not writing into the site. Drafted HTML length:", html.length, "chars");
    console.log("---");
  ...
  // 5) Write the post into the repo root (the static site). The workflow
  // commits + pushes and the host auto-deploys.
  assertSiteDir();
  ...
  // 7) Save state.json in THIS (blog-agent) folder so the next run skips this topic.
